...
int energy_consumption::init2(OBJECT *parent)
{
	...
	pos_minute[0]=1;
	myfile2 <<pos_minute[0]<<endl;
	for(ii=1;ii<number_of_stations;ii++)
	{
		pos_minute[ii]=(int)(pos_minute[ii-1]+((pos_station[ii]*1000-pos_station[ii-1]*1000) /(speed*0.277))/60);
...
TIMESTAMP energy_consumption::sync(TIMESTAMP t0, TIMESTAMP t1) 
{
		...
		for(i=0;i<number_of_stations;i++)
		{
			if(elevation2>0)
			{
				...
				else if(counter == pos_minute[i])acceleration = ((speed*0.277)-0)/60;
				else if(counter == pos_minute[i]-1)acceleration = ((0-speed*0.277))/60;
				else acceleration=0;
			}
		...
		//energy consumption
		if(elevation2>0)
		power_consumed =(*Ener_Lights)+(*Ener_AC)+ (0.000000278)*30*(speed*0.277)*(0.5*p*2*cl*A*(speed*0.277+vwind)*(speed*0.277+vwind) +cr*weight*9.82 + weight*(acceleration) + weight * 9.82 *(elevation/1000) );
		else
		{
		power_consumed =(*Ener_Lights)+(*Ener_AC)+(0.000000278)*30*(speed*0.277)*( 0.5*p*2*cl*A*(speed*0.277+vwind)*(speed*0.277+vwind) +cr*weight*9.82 +weight*(acceleration)  );
		sum_brake = sum_brake - 0.000000278* 60*(speed*0.277)*weight*(acceleration);
		brake_file << sum_brake <<"\n";
