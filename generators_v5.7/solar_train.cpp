...
int *station;					//stations of the route
...
//in order to use wind turbine object
OBJECT *obj6;
double *wind_energy;
...
ofstream myfile3; //boithiktiko arxeio

double total_energy_brake=0;
double power_consumed2;
complex more_energy;
...
int solar_train::init2(OBJECT *parent)
{
	...
	{
	    if(e_per_station[k+1].r < 0 && e_per_station[k].r< 0)
		{
			total_energy_brake= total_energy_brake + ((-1)*e_per_station[k+1].r + e_per_station[k].r );
	...
	for(k=0;k<num_stations-1;k++)
	{
		if(e_per_station[k+1].r < 0)
               energy_bet_stations[k]=e_per_station[k];
		else
	...
	char *inname3 = "brake.txt";
		
    ifstream infile2(inname2);
	ifstream infile3(inname3);
...
TIMESTAMP solar_train::sync(TIMESTAMP t0, TIMESTAMP t1)
{
		...
		double vwind=5.25;					 //wind's speed
		...
		wind_energy=gl_get_double_by_name(obj6,"Pconv");
		...
		for(i=0;i<num_stations;i++)
		{
			if(elevation2>0)
			{   
				if(counter==1)acceleration = ((speed*0.277)-0)/60;
				else if(counter == station[i])acceleration = ((speed*0.277)-0)/60;
				else if(counter == station[i]-1)acceleration = (0-(speed*0.277))/60;
				else acceleration=0;
			}
		...
		//energy consumption
		if(elevation2>0)
			 power_consumed =(*EnergyAC)+(*EnergyLights)+(0.000000278)*30*(speed*0.277)*( 0.5*p*2*cl*A*(speed*0.277+vwind)*(speed*0.277+vwind) +cr*weight*9.82 + weight*(acceleration) + weight * 9.82 *(elevation/1000) );
		else
		{
			power_consumed =(*EnergyAC)+(*EnergyLights)+(0.000000278)*30*(speed*0.277)*( 0.5*p*2*cl*A*(speed*0.277+vwind)*(speed*0.277+vwind) +cr*weight*9.82 + weight*(acceleration) );
		}  
		...
		//charge the battery from wind turbines if the panels cannot give you the appropriate energy
		if((*pv_out2).r > 15)
		{
			electric_energy_per_minute=(*pv_out2/1000)*60;	//pv_out2
			//printf("pv : %f \n",(electric_energy_per_minute));		
		}
		else
		{
			electric_energy_per_minute=((*wind_energy)/1000)*60;
			//printf("wind_turbine : %f \n",electric_energy_per_minute);	
	    }

		//Discharging of train's battery
		Energy1 = Energy1 - ( power_consumed)*(1/efficiency);//+electric_energy_per_minute*(1 /efficiency);
		if( Energy1.r > initial_energy.r ){Energy1.r = initial_energy.r;}
          myfile3 << Energy1.r <<"\n";
    	...
    	for(i=1;i<num_stations+1; i++)
		{
				...
				if(counter < station[i])
				{
					...
					current_station = i;
					station1=station[i-1]; //se lepta
					station2=station[i];
					
            
						 energy_between_stations1=energy_bet_stations[i];
					
					
					 if(i== num_stations-1)
						 energy_between_stations2=0;
					 else 
						 energy_between_stations2=energy_bet_stations[i+1];
			...
			for(i=0;i<num_stations;i++)
			{
				if(i!=current_station)
				{
					if(Energy_station[i] + error <  initial_energy.r)
					{
						...
						if( Energy_station[i] > initial_energy.r ){Energy_station[i]=initial_energy.r ; }
					}
				}
			}
		}
		else 
		{
			for(i=0;i<num_stations;i++)
			{
				Energy_station[i] = initial_energy.r;
		...
		ektimisi=Energy_station[current_station].r + electric_energy_per_minute.r*(station2-counter); //ektimhsh gia ton stathmo pou fthanw an mporei h energeia na me paei sthn epomenh stash
	 
		
		if(ektimisi.r > initial_energy.r)
		{
		more_energy =  electric_energy_per_minute.r*(station2-counter);
		ektimisi = initial_energy.r;
		
		}
	// if(Energy1.r <  remainenergy.r ){printf("edw_031 \n");counter=station1;change=1;}
		//determines when to shift from charging by pv panels to charging by power plants
	    if(counter < station2 && counter > station1)
	    {
				
			if(Energy1 +total_energy_brake - energy_between_stations2.r < remainenergy.r ){change=1;plug_in=1;}
			
			else if(Energy1.r +total_energy_brake <  remainenergy.r ){counter=station1;change=1;}
			else if(Energy1.r <  remainenergy.r ){counter=station1;change=1;}
			else if(Energy1.r + total_energy_brake > (energy_between_stations1.r/(station2-counter)) + energy_between_stations2.r )
			{
			...
			else if(Energy1.r > (energy_between_stations1.r/(station2-counter)) + energy_between_stations2.r )
			{
